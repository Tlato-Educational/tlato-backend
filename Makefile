...
start/docker/db: ## Start database container
	...
	@$(DOCKER_COMPOSE) -f $(DOCKER_COMPOSE_FILE) --env-file .env up -d express-typescript-template-postgres express-typescript-template-pgweb

.PHONY: stop/docker/db
stop/docker/db: ## Stop database container
	@echo "🛑 Stopping database (Docker)..."
	@$(DOCKER_COMPOSE) -f $(DOCKER_COMPOSE_FILE) --env-file .env stop express-typescript-template-postgres express-typescript-template-pgweb

.PHONY: start/docker/cache
start/docker/cache: ## Start cache container
	@echo "▶️ Starting cache (Docker)..."
	@$(DOCKER_COMPOSE) -f $(DOCKER_COMPOSE_FILE) --env-file .env up -d express-typescript-template-redis express-typescript-template-redis-commander

.PHONY: stop/docker/cache
stop/docker/cache: ## Stop cache container
	@echo "🛑 Stopping cache (Docker)..."
	@$(DOCKER_COMPOSE) -f $(DOCKER_COMPOSE_FILE) --env-file .env stop express-typescript-template-redis express-typescript-template-redis-commander
